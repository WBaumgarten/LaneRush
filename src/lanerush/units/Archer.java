...
public class Archer extends Unit {
    ...
    public Archer(ID id, boolean playerLeft, int xCoord, int yCoord, int lane, Handler handler, SpriteSheet ss) {
        ...
        if (isPlayerLeft()) {
            stillImg = ss.grabImage(1, 5, 64, 64);
            walkImg[0] = ss.grabImage(3, 5, 64, 64);
            walkImg[1] = ss.grabImage(5, 5, 64, 64);
            walkImg[2] = ss.grabImage(7, 5, 64, 64);
            walkImg[3] = ss.grabImage(5, 5, 64, 64);
        } else {
            stillImg = ss.grabImage(9, 5, 64, 64);
            walkImg[0] = ss.grabImage(11, 5, 64, 64);
            walkImg[1] = ss.grabImage(13, 5, 64, 64);
            walkImg[2] = ss.grabImage(15, 5, 64, 64);
            walkImg[3] = ss.grabImage(13, 5, 64, 64);
        }
    ...
    public Rectangle getAttackBounds() {
        if (playerLeft) {
            return new Rectangle(x + 32, y - 48, 96, 128);
        } else {
            return new Rectangle(x - 96, y - 48, 96, 128);
        }
    }
...
//        g.fillRect(x + 12, y + 12, 8, 8);
    ...
    public void attack() {
        ...
        if (currTime - cooldownTimer > cooldown) {
            if (target != null) {
                handler.addObject(new Arrow(ID.Projectile, playerLeft, damage, this, target, handler, ss));
